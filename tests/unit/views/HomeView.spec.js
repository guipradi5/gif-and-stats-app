...
jest.mock('@/api/api', () => ({
  ...
  getGifs: () => [{id:5, description: "gif"},{id:6, description: "gif2"},{id:7, description: "gif3"}],
  getGifClickCount: () => 15
}));
...
describe("HomeView.vue", () => {
  ...
  it('HomeView should render', async() => {
    const wrapper = await component()
    await wrapper.vm.$nextTick()
    await wrapper.findAllComponents(GifListElement).at(1).trigger("select", {id:6, description: "gif2"})
    await wrapper.vm.$nextTick()
