...
export const getTypeQuantity = (coupons, type) => {
  ...
  coupons.forEach((coupon) => {
    if (type === coupon.promotion_type) count++;
  });
  return count;
};
...
const getPropertyMinValue = (coupons, property, propertyValue) => {
  ...
  coupons.forEach((coupon) => {
    if (propertyValue === coupon[property]) {
      if (!min || coupon.value < min) min = coupon.value;
    }
  });
  return min;
};
...
export const getRetailersData = (coupons) => {
    ...
    if (!retailers.includes(coupon[property])) {
      ...
      result = {
        ...
        [coupon[property]]: {
          ...
          percentOff: {
            count: 1,
            minValue: coupon.value,
            maxValue: coupon.value || 0,
      ...
      if (coupon.value) {
        ...
        if (type) {
          ...
          if (
            !resultRetailer[type].minValue ||
            (coupon.value && coupon.value < resultRetailer[type].minValue)
          ) {
  ...
  retailers.forEach((retailer) => {
    result[retailer].percentOff.average = (
      result[retailer].percentOff.average / result[retailer].percentOff.count
    ).toFixed(2);
    result[retailer].dollarOff.average = (
      result[retailer].dollarOff.average / result[retailer].dollarOff.count
    ).toFixed(2);
  });
  return result;
};
